...
plotQuantileRegressionOT2D<-function(QRObject,contours=TRUE,rays=FALSE,g_row,g_col){
  library(scales)
  center<-QRObject$center
  par(mfrow=c(g_row,g_col),mar=c(1.7, 1.7, 0.5, 0.5),oma = c(0.5, 0.5, 0.5, 0.5))
  maxY1<-minY1<-maxY2<-minY2<-NULL
  for (i in 1:QRObject$grid){
    maxY1<-max(maxY1,QRObject$quantiles[[i]]$quantile3[,2])
    minY1<-min(minY1,QRObject$quantiles[[i]]$quantile3[,2])
    maxY2<-max(maxY2,QRObject$quantiles[[i]]$quantile3[,3])
    minY2<-min(minY2,QRObject$quantiles[[i]]$quantile3[,3])
  }
  for(i in 1:QRObject$grid){
    plot(QRObject$quantiles[[i]]$quantile3[,2],
         QRObject$quantiles[[i]]$quantile3[,3],
         type='l',col='white',
         xlim=c(minY1,maxY1),ylim=c(minY2,maxY2))
    if(contours==TRUE){
    lines(QRObject$quantiles[[i]]$quantile3[,2],
         QRObject$quantiles[[i]]$quantile3[,3],
         col='darkolivegreen1',lwd=2)
    lines(QRObject$quantiles[[i]]$quantile2[,2],
          QRObject$quantiles[[i]]$quantile2[,3],
          col='darkolivegreen3',lwd=2)
    lines(QRObject$quantiles[[i]]$quantile1[,2],
          QRObject$quantiles[[i]]$quantile1[,3],
          col='darkolivegreen',lwd=2)
    }
    if(rays==TRUE){
      lines(QRObject$quantiles[[i]]$ray1[,2],
            QRObject$quantiles[[i]]$ray1[,3],
            col=alpha('magenta3',0.3),lwd=2)
      lines(QRObject$quantiles[[i]]$ray2[,2],
            QRObject$quantiles[[i]]$ray2[,3],
            col=alpha('magenta3',0.3),lwd=2)
      lines(QRObject$quantiles[[i]]$ray3[,2],
            QRObject$quantiles[[i]]$ray3[,3],
            col=alpha('magenta3',0.3),lwd=2)
      lines(QRObject$quantiles[[i]]$ray4[,2],
            QRObject$quantiles[[i]]$ray4[,3],
            col=alpha('magenta3',0.3),lwd=2)
      lines(QRObject$quantiles[[i]]$ray5[,2],
            QRObject$quantiles[[i]]$ray5[,3],
            col=alpha('magenta3',0.3),lwd=2)
      lines(QRObject$quantiles[[i]]$ray6[,2],
            QRObject$quantiles[[i]]$ray6[,3],
            col=alpha('magenta3',0.3),lwd=2)
      lines(QRObject$quantiles[[i]]$ray7[,2],
            QRObject$quantiles[[i]]$ray7[,3],
            col=alpha('magenta3',0.3),lwd=2)
      lines(QRObject$quantiles[[i]]$ray8[,2],
...
plotQuantileRegressionOT3Dcentral<-function(QRObject){
  library(rgl)
  center<-QRObject$center
  lines3d(x=center[,1],y=center[,2],z=center[,3],col='red',lwd=2)
  for(i in 1:QRObject$grid){
    lines3d(x=QRObject$quantiles[[i]]$quantile1[,1],
            y=QRObject$quantiles[[i]]$quantile1[,2],
            z=QRObject$quantiles[[i]]$quantile1[,3],
            col='darkgreen',add=TRUE,lwd=2,alpha=0.6)
  }
  for (j in 1:QRObject$ContLength){
    edge<-NULL
    for(i in 1:QRObject$grid){
      edge<-rbind(edge,QRObject$quantiles[[i]]$quantile1[j,])
    }
    lines3d(x=edge[,1],
            y=edge[,2],
            z=edge[,3],
  ...
  for(i in 1:QRObject$grid){
    lines3d(x=QRObject$quantiles[[i]]$quantile2[,1],
            y=QRObject$quantiles[[i]]$quantile2[,2],
            z=QRObject$quantiles[[i]]$quantile2[,3],
            col='darkolivegreen3',add=TRUE,lwd=2,alpha=0.6)
  }
  for (j in 1:QRObject$ContLength){
    edge<-NULL
    for(i in 1:QRObject$grid){
      edge<-rbind(edge,QRObject$quantiles[[i]]$quantile2[j,])
    }
    lines3d(x=edge[,1],
            y=edge[,2],
            z=edge[,3],
  ...
  for(i in 1:QRObject$grid){
    lines3d(x=QRObject$quantiles[[i]]$quantile3[,1],
            y=QRObject$quantiles[[i]]$quantile3[,2],
            z=QRObject$quantiles[[i]]$quantile3[,3],
            col='darkolivegreen1',add=TRUE,lwd=2,alpha=0.6)
  }
  for (j in 1:QRObject$ContLength){
    edge<-NULL
    for(i in 1:QRObject$grid){
      edge<-rbind(edge,QRObject$quantiles[[i]]$quantile3[j,])
    }
    lines3d(x=edge[,1],
            y=edge[,2],
            z=edge[,3],
